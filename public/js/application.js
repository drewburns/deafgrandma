$(document).ready(function(){                                  // this means, only do this when the document  is ready
  $("#user_input").on("submit", function(event) {     // do this function when element w/ id user_input is submitted
    ...
    console.log(response)
       $("#grandma-says").fadeIn()      // you can use $.ajax or $.post. it submits a post requst with the specified url and data
      $("#grandma-says").html(response)
      $("#grandma-says").fadeOut(3000)
        // and get a response. this line takes the response and puts it in the html of element with id = 
                                                                        //  grandma-says
    });
  });
});